...
// ---------------------------------------------------------------
// Crop health triage widget — photo in, plain-language triage out.
// Scoped deliberately: this calls /api/crop-triage (see
// lib/crop-triage.js), which never returns a specific chemical name,
// dosage, or timing — only plausible issue names, general cultural-
// practice guidance, and a pointer to a real KVK/helpline. The
// district's water-stress band shown alongside the result is this
// project's own deterministic index, not something the model guessed.
// ---------------------------------------------------------------
function renderCropTriageDropdown() {
  const el = document.getElementById('cropTriageDistrict');
  if (!el) return;
  el.innerHTML = '';
  appleDistricts().forEach(d => {
    const opt = document.createElement('option');
    opt.value = d.id;
...
function renderCropTriageResult(result, district) {
  const box = document.getElementById('cropTriageResult');
  if (!box) return;
  box.hidden = false;

  if (result.error) {
    box.innerHTML = `<p class="crop-triage-error">${t('crop_triage_error')}</p>`;
    return;
  }

  const isLive = result.mode === 'live';
  const badgeHtml = `<span class="crop-triage-result-badge ${isLive ? 'is-live' : 'is-demo'}">${isLive ? t('crop_triage_live_badge') : t('crop_triage_demo_badge')}</span>`;

  const confidenceHtml = `<div class="crop-triage-confidence">${t('crop_triage_confidence_label')}: <b>${result.confidence || '—'}</b></div>`;

  const issuesHtml = (result.likely_issues && result.likely_issues.length)
    ? `<h5>${t('crop_triage_issues_title')}</h5><ul>${result.likely_issues.map(i => `<li>${i}</li>`).join('')}</ul>`
    : '';

  const guidanceHtml = (result.general_guidance && result.general_guidance.length)
    ? `<h5>${t('crop_triage_guidance_title')}</h5><ul>${result.general_guidance.map(g => `<li>${g}</li>`).join('')}</ul>`
    : '';

  const noteHtml = result.notes ? `<p class="crop-triage-result-note">${result.notes}</p>` : '';

  let waterNoteHtml = '';
  if (district && (district.status === 'stress' || district.status === 'watch')) {
    const tmpl = CROP_TRIAGE_WATER_NOTE[district.status][currentLang] || CROP_TRIAGE_WATER_NOTE[district.status].en;
    waterNoteHtml = `<div class="crop-triage-water-note">${tmpl(district.name.split(',')[0])}</div>`;
  }

  const helplineHtml = `<div class="crop-triage-helpline"><strong>${t('crop_triage_helpline_title')}</strong><br>${t('crop_triage_helpline_text')}</div>`;
  const disclaimerHtml = `<p class="crop-triage-disclaimer">${t('crop_triage_disclaimer')}</p>`;

  box.innerHTML = badgeHtml + confidenceHtml + waterNoteHtml + issuesHtml + guidanceHtml + noteHtml + helplineHtml + disclaimerHtml;
}

function initCropTriageWidget() {
  const widget = document.getElementById('cropTriageWidget');
  const toggle = document.getElementById('cropTriageToggle');
  const photoInput = document.getElementById('cropTriagePhoto');
  const previewWrap = document.getElementById('cropTriagePreviewWrap');
  const preview = document.getElementById('cropTriagePreview');
  const submitBtn = document.getElementById('cropTriageSubmit');
  const districtSelect = document.getElementById('cropTriageDistrict');
  const resultBox = document.getElementById('cropTriageResult');
  if (!widget || !toggle || !submitBtn) return;

  renderCropTriageDropdown();
  ...
  submitBtn.addEventListener('click', async () => {
    ...
    if (!file) {
      ...
      resultBox.innerHTML = `<p class="crop-triage-error">${t('crop_triage_select_photo')}</p>`;
      return;
    }

    const originalLabel = submitBtn.textContent;
    submitBtn.disabled = true;
    submitBtn.textContent = t('crop_triage_analyzing');
    resultBox.hidden = true;

    try {
      const imageBase64 = await fileToBase64(file);
      const res = await fetchWithTimeout('/api/crop-triage', 25000, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ imageBase64, mediaType: file.type || 'image/jpeg', lang: currentLang })
      });
      const payload = await res.json();
      if (!res.ok) throw new Error(payload.error || `status ${res.status}`);
      const district = appleDistricts().find(d => d.id === districtSelect.value);
      renderCropTriageResult(payload, district);
    } catch (err) {
      renderCropTriageResult({ error: String(err) });
    } finally {
