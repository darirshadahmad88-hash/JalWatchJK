...
function formatTelegramMessage({ district, waterSignal, mandiPrice, floodStatus, asOf }) {
  ...
  const waterLine = waterSignal.type === 'soil_moisture'
    ? `Soil moisture: ${waterSignal.value ?? 'n/a'}% ${waterSignal.mode === 'demo' ? '(demo)' : '(live)'}`
    : `Groundwater level: ${waterSignal.value ?? 'n/a'} m below ground ${waterSignal.mode === 'demo' ? '(demo)' : '(live)'}`;

  const priceLine = `Mandi price (${mandiPrice.commodity}): ₹${mandiPrice.modal_price}/quintal ` +
    `${mandiPrice.mode === 'demo' ? '(demo)' : `(live, ${mandiPrice.market})`}`;

  const floodLine = `Jhelum flood status: ${floodStatus.severity}${floodStatus.mode === 'demo' ? ' (demo)' : ''}`;

  return [
    `${stress.emoji} *${district.name}* — Water Stress: ${stress.label}`,
    waterLine,
    priceLine,
    floodLine,
    '',
    'This is a current-conditions signal, not a forecast.',
    `As of: ${asOf}`
  ].join('\n');
}
